...
while Etat=='perdant':
    ...
    # il est préférable de jouer sur des grilles de tailles inférieures ou égales à 9x9 car à partir de 10x10 les numéros des cases passent à 3 caractères(ex : 100) ce qui rend difficile la lecture des grilles
    while n>=10 or n<=2 :
            n = int(input("n est un entier positif compris entre 3(inclus) et 9 (inclus): n="))
    ...
    if n==3:
        ...
        if réponse=="IG":
            interfacegraphique()
            k=0 # sert à sortir de la boucle while
    if k==0:
        Etat='gagnant' #on sort de la boucle while
